#!/usr/bin/env python
"""
Drop-cube experiment — sweep height x tilt-angle, classify
which of the 6 faces lands down, and show the outcome map.

Usage:
    python experiments/drop_cube.py              # defaults
    python experiments/drop_cube.py --workers 4  # limit CPU cores
    python experiments/drop_cube.py --axis z     # tilt about z-axis
"""
...
def main():
    parser = argparse.ArgumentParser(description="Cube drop outcome map")
    ...
    sweep_drop("cube", heights, angles, tilt_axis=args.axis,
    ...
    fig, ax, img = plot_drop_map(heights, angles, results.astype(int),
                                 "cube", args.axis)
    ax.set_title(f"cube drop outcome map — tilt about {args.axis}-axis")
    plt.show()
