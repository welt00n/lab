...
Usage:
    python experiments/drop_rod.py              # defaults
    python experiments/drop_rod.py --workers 4  # limit CPU cores
    python experiments/drop_rod.py --axis z     # tilt about z-axis
"""
...
def main():
    parser = argparse.ArgumentParser(description="Rod drop outcome map")
    ...
    sweep_drop("rod", heights, angles, tilt_axis=args.axis,
    ...
    fig, ax, img = plot_drop_map(heights, angles, results.astype(int),
                                 "rod", args.axis)
    ax.set_title(f"rod drop outcome map — tilt about {args.axis}-axis")
    plt.show()
